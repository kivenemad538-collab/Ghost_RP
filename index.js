...
function ticketRows(num) {
  return [
    ...
    new ActionRowBuilder().addComponents(
      ...
      new ButtonBuilder().setCustomId(`ticket_copy:${num}`).setLabel('ÙØ³Ø®').setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId(`ticket_delete:${num}`).setLabel('ÙØ³Ø­').setStyle(ButtonStyle.Danger)
    )
  ];
}
...
async function closeTicketNow(interaction,ticket,reason='ØªÙ Ø§ÙØ¥ØºÙØ§Ù Ø¨ÙØ§Ø³Ø·Ø© Ø§ÙØ¥Ø¯Ø§Ø±Ø©') {
  ...
  await interaction.reply({content:`ð ØªÙ Ø¥ØºÙØ§Ù Ø§ÙØªØ°ÙØ±Ø©.\nØ§ÙØ³Ø¨Ø¨: ${reason}`});
  await safeDM(ticket.ownerId,{
    embeds:[embed('ð ØªÙ Ø¥ØºÙØ§Ù Ø§ÙØªØ°ÙØ±Ø©',`#${ticket.number} | ${CONFIG.TICKET_TYPES[ticket.type].label}\nØ§ÙØ³Ø¨Ø¨: ${reason}`)],
    components:[new ActionRowBuilder().addComponents(
      ...[1,2,3,4,5].map(n=>new ButtonBuilder().setCustomId(`ticket_rate:${ticket.number}:${n}`).setLabel(`${n} â­`).setStyle(ButtonStyle.Secondary))
    )]
  });
  await ticketLog('Ø¥ØºÙØ§Ù ØªØ°ÙØ±Ø©',`#${ticket.number}\nØ¨ÙØ§Ø³Ø·Ø© <@${interaction.user.id}>\nØ§ÙØ³Ø¨Ø¨: ${reason}`,0xE74C3C);
}
...
async function reopenTicketNow(interaction,ticket) {
  ...
  await interaction.reply({content:'â»ï¸ ØªÙ Ø¥Ø¹Ø§Ø¯Ø© ÙØªØ­ Ø§ÙØªØ°ÙØ±Ø©.'});
  await safeDM(ticket.ownerId,{embeds:[embed('â»ï¸ Ø¥Ø¹Ø§Ø¯Ø© ÙØªØ­ Ø§ÙØªØ°ÙØ±Ø©',`ØªÙ Ø¥Ø¹Ø§Ø¯Ø© ÙØªØ­ ØªØ°ÙØ±ØªÙ #${ticket.number}.`)]});
}
...
async function ticketTranscript(interaction,ticket) {
  ...
  await interaction.reply({content:'ð ÙØ³Ø®Ø© Ø§ÙØªØ°ÙØ±Ø©:',files:[file],ephemeral:true});
}
...
async function checkTicketWarnings() {
  ...
  for(const t of Object.values(db.tickets||{})){
    ...
    if(!ch) continue;
    t.status='closed'; t.warningDeadline=0; t.closeReason='Ø¹Ø¯Ù Ø§ÙØªÙØ§Ø¹Ù Ø®ÙØ§Ù 24 Ø³Ø§Ø¹Ø©'; saveDB();
    await ch.permissionOverwrites.edit(t.ownerId,{ViewChannel:false,SendMessages:false}).catch(()=>{});
    await ch.send({content:'ð ØªÙ Ø§ÙØ¥ØºÙØ§Ù ØªÙÙØ§Ø¦ÙØ§Ù Ø¨Ø³Ø¨Ø¨ Ø¹Ø¯Ù Ø§ÙØªÙØ§Ø¹Ù Ø®ÙØ§Ù 24 Ø³Ø§Ø¹Ø©.'}).catch(()=>{});
    ...
    await ticketLog('Ø¥ØºÙØ§Ù ØªÙÙØ§Ø¦Ù',`#${t.number} - Ø¹Ø¯Ù Ø§ÙØªÙØ§Ø¹Ù 24 Ø³Ø§Ø¹Ø©`,0xE74C3C);
  }
}
...
async function decideSimple(interaction,kind,accept) {
  if(!isReviewer(interaction.member)&&!isControl(interaction.member)) return interaction.reply({content:'â ÙÙØ³ ÙØ¯ÙÙ ØµÙØ§Ø­ÙØ©.',ephemeral:true});
  ...
  const msg=kind==='staff'&&accept?`ØªÙ ÙØ¨ÙÙÙ ÙØ¨Ø¯Ø¦ÙØ§Ù. ÙÙØ§Ø¹ÙØ¯ Ø§ÙÙÙØ§Ø¨ÙØ©: ${channelUrl(CONFIG.STAFF_INTERVIEW_SCHEDULE_CHANNEL_ID)}`:(accept?'ØªÙ ÙØ¨ÙÙÙ ÙØªÙØª Ø¥Ø¶Ø§ÙØ© Ø§ÙØ±ØªØ¨Ø©.':'ØªÙ Ø±ÙØ¶ Ø§ÙØªÙØ¯ÙÙ.');
  await safeDM(data.userId,{embeds:[embed(accept?'â ØªÙ Ø§ÙÙØ¨ÙÙ':'â ØªÙ Ø§ÙØ±ÙØ¶',msg,accept?0x2ECC71:0xE74C3C)]});
  const e=EmbedBuilder.from(interaction.message.embeds[0]).setColor(accept?0x2ECC71:0xE74C3C).addFields({name:'Ø§ÙÙØ±Ø§Ø±',value:`${accept?'â ÙØ¨ÙÙ':'â Ø±ÙØ¶'} Ø¨ÙØ§Ø³Ø·Ø© <@${interaction.user.id}>`});
...
client.on(Events.InteractionCreate,async interaction=>{
  try{
    if(interaction.isButton()){
      ...
      if(id.startsWith('ticket_delete:')){const t=db.tickets[id.split(':')[1]];if(!ticketManagementStaff(interaction.member))return interaction.reply({content:'â ÙØ±ÙÙ Ø§ÙØªØ°Ø§ÙØ± Ø£Ù ÙØ³Ø¤ÙÙ Ø§ÙØªØ°Ø§ÙØ± ÙÙØ·.',ephemeral:true});await interaction.reply({content:'ðï¸ Ø³ÙØªÙ Ø§ÙÙØ³Ø­ Ø®ÙØ§Ù 5 Ø«ÙØ§ÙÙ.'});t.status='deleted';saveDB();return setTimeout(()=>interaction.channel.delete().catch(()=>{}),5000);}
